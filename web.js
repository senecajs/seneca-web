...
module.exports = function( options ) {
  ...
  var web = function( req, res, next ) {
    ...
    next_service(req,res,function(err, response){
      if (err){
        var http = {}

        http.redirect = err.http$ && err.http$.redirect
        http.status   = err.http$ && err.http$.status || 400
        ...
        if( http.redirect ) {
          ...
          delete err.http$
          res.status(http.status).send(err)
        }
        res.end()
      }
      else{
